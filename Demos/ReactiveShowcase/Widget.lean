/-
  ReactiveShowcase - Widget Visual Builders
  Adapted from CanopyShowcase to use ReactiveShowcaseSnapshot.
-/
import Afferent
import Afferent.Canopy
import Trellis
import Demos.ReactiveShowcase.State
import Demos.ReactiveShowcase.Network
...
/-- Build a visual button (renders state from snapshot). -/
...
/-- Build a visual checkbox (renders state from snapshot). -/
...
/-- Build the Reactive showcase widget with interactive elements. -/
def reactiveShowcaseWidget (fontId : FontId) (smallFontId : FontId)
    (screenScale : Float := 1.0) (state : ReactiveShowcaseSnapshot) : WidgetBuilder := do
  ...
  column (gap := s 20) (style := {
    ...
    -- Title (changed from "Canopy Widgets" to "Reactive Showcase")
        ...
        titledPanel "Text Inputs" .outlined theme do
          column (gap := s 12) (style := {}) #[
            ...
            textInputVisual textInput1Name theme
              { state.textInput1 with focused := state.focusedInput == some textInput1Name }
              "Enter text here...",
            textInputVisual textInput2Name theme
              { state.textInput2 with focused := state.focusedInput == some textInput2Name }
              "Type something..."
          ],
        ...
        titledPanel "Text Area" .outlined theme do
          column (gap := s 8) (style := {}) #[
            ...
            textAreaVisual textAreaName theme
              { state.textArea with focused := state.focusedInput == some textAreaName }
